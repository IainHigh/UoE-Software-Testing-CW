...
1. **Performance**
   ...
   - 2000 moves (battery capacity) in this limit of 60 seconds.
...
2. **Accuracy**
   ...
   - For invalid orders, the correct reason must be stated.
   - For valid orders, all valid orders MUST be delivered except in the case the drone runs out of battery.

3. **Battery Efficiency**
   - Limit drone moves to 2000 per session to conserve battery life.
   - Optimise flight paths to minimise energy consumption.
...
1. **Usability**
   ...
   - Provide clear error messages should the program crash unexpectedly.
...
2. **Maintainability**
   ...
   - Ensure loose coupling so if one feature has to be changed there is minimal dependencies to resolve.
