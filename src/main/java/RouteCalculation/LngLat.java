...
public class LngLat {
     ...
     * Draws a horizontal line from the point to the right and counts the number of
     * times it intersects with the border.
     * If the number of intersections is odd, the point is inside the zone.
    ...
    public boolean inZone(LngLat[] zoneCoordinates) {
        ...
        for (int i = 0; i < zoneCoordinates.length; i++) {
            ...
            // If it's above the lower point and below the upper point and lies to the left
            // of the line between
     ...
     * Gets the central area border coordinates from the singleton class and calls
     * the inZone method.
     *
    ...
    /**
     * Checks if the line between the current point and the previous point is in a
     * no-fly zone.
     *
     * @param previousPoint The previous point in the path.
     * @return True if the line between the current point and previous point is in a
     *         no-fly zone, false otherwise.
     */
    public boolean inNoFlyZone(LngLat previousPoint) {
        ...
        for (LngLat[] noFlyZone : AreaSingleton.getInstance().getNoFlyZones()) {
            ...
            // If the line between the current point and the previous point intersects with
            // the no-fly zone, return true.
    ...
    /**
     * Calculates the distance between two points and checks if it is within the
     * defined definition of "close" (0.00015)
     *
    ...
    /**
     * Given a compass direction, calculates the drone's next position using
     * trigonometry.
     *
    ...
    /**
     * Uses the RouteCalculator to calculate the shortest route from this point to
     * the destination.
     *
     * @param destination The LngLat point we are trying to reach.
     * @return An array of CompassDirections representing the shortest path from
    ...
    /**
     * Uses the RouteCalculator to calculate the shortest route from this point to
     * the destination. Then counts the
    ...
    public int numberOfMovesTo(LngLat destination) {
        ...
        if (route == null) {
            // If the route is null, then this means we can't find a path to the
            // destination, and so we can treat it
